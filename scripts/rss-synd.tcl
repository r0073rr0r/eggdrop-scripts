...
#
#   5. Debug Logging:
#      - Added optional debug logging to #services channel (enabled by default)
#      - Controlled by "debug" setting (1=enabled, 0=disabled) in feed config
#      - Can be set per-feed or globally in default settings
#      - Debug channel configurable via "debug-channel" setting (default: #services)
...
#
# CONFIGURATION:
#   - Feed settings: Configured directly in this file (rss() array)
#   - Database location: scripts/feeds/ (created automatically)
#   - Debug logging: Enabled by default, sends to #services channel
#     * Set "debug" 0 to disable debug logging
#     * Set "debug-channel" to change debug output channel
...
namespace eval ::rss-synd {
	...
	# Debug settings:
	#   "debug" - Enable/disable debug logging (1=enabled, 0=disabled)
	#   "debug-channel" - Channel where debug messages are sent (default: #services)
	#   These can be overridden per-feed by adding them to individual feed configs
}
...
# Helper function to find feed name by database path
proc ::rss-synd::find_feed_name {database_path} {
	variable rss
	foreach name [array names rss] {
		array set feed $rss($name)
		if {$feed(database) == $database_path} {
			return $name
		}
		unset feed
	}
...
# Debug logging to #services channel (optional, controlled by debug setting)
proc ::rss-synd::debug_log {msg {feed_name ""}} {
	variable rss
	variable default
	
	# Check if debug is enabled (default or per-feed)
	set debug_enabled 1
	set debug_channel "#services"
	
	if {$feed_name != "" && [info exists rss($feed_name)]} {
		array set feed $rss($feed_name)
		if {[info exists feed(debug)]} {
			set debug_enabled $feed(debug)
		}
		if {[info exists feed(debug-channel)] && $feed(debug-channel) != ""} {
			set debug_channel $feed(debug-channel)
		}
		unset feed
	} else {
		# Use default settings
		array set def $default
		if {[info exists def(debug)]} {
			set debug_enabled $def(debug)
		}
		if {[info exists def(debug-channel)] && $def(debug-channel) != ""} {
			set debug_channel $def(debug-channel)
		}
		unset def
	}
	
	if {$debug_enabled} {
		if {[validchan $debug_channel] && [botonchan $debug_channel]} {
			putserv "PRIVMSG $debug_channel :\002RSS Debug\002: $msg"
		}
		putlog "\002RSS Debug\002: $msg"
	}
}
			...
			if {[info exists rss($feed_id)]} {
				...
				if {[catch {set data [[namespace current]::feed_read]} error] == 0} {
					...
					if {[catch {set token [::http::geturl "$feed(url)" -command "[namespace current]::feed_callback {[array get feed] depth 0}" -timeout $feed(timeout) -headers $feed(headers)]} http_error]} {
						...
						[namespace current]::debug_log "Started HTTP request for '$feed_id', token: $token" $feed_id
					}
					return
					}
			...
			if {[catch {set data [[namespace current]::feed_read]} error] == 0} {
				if {[string length $data] == 0} {
					...
					if {[catch {set token [::http::geturl "$feed(url)" -command "[namespace current]::feed_callback {[array get feed] depth 0}" -timeout $feed(timeout) -headers $feed(headers)]} http_error]} {
						...
						[namespace current]::debug_log "Started HTTP request for '$name', token: $token" $name
					}
					continue
				}
				if {![[namespace current]::feed_info $data]} {
...
proc ::rss-synd::feed_callback {feedlist args} {
	...
	set feed_name [[namespace current]::find_feed_name $feed(database)]
	[namespace current]::debug_log "feed_callback called for URL: $state(url), status: $state(status)" $feed_name
	...
	if {[catch {
		...
		set feed_name [[namespace current]::find_feed_name $feed(database)]
		[namespace current]::debug_log "Got HTTP data, length: [string length $data]" $feed_name
		...
		if {[info exists feed(feedencoding)]} {
			...
			[namespace current]::debug_log "Converted from feedencoding: $feed(feedencoding)" $feed_name
		}
		...
		if {[info exists feed(charset)]} {
			if {[string tolower $feed(charset)] == "utf-8" && [is_utf8_patched]} {
				...
				[namespace current]::debug_log "Converted to charset: $feed(charset)" $feed_name
			}
		}
		...
		if {[info exists meta(Content-Encoding)]} {
			...
			if {[string equal $content_encoding "gzip"]} {
				[namespace current]::debug_log "Content is gzip encoded, attempting decompression" $feed_name
				if {[catch {set data [[namespace current]::feed_gzip $data]} err]} {
					...
					putlog "\002RSS Info\002: Feed claims gzip compression but decompression failed (server may have sent uncompressed data). Continuing with plain XML parsing."
					# Continue with the data as-is, it might not actually be compressed
				} else {
					[namespace current]::debug_log "Successfully decompressed gzip data" $feed_name
				}
			}
		}

		[namespace current]::debug_log "Attempting to parse XML, data length: [string length $data]" $feed_name
	} err]} {
	...
	set feed_name [[namespace current]::find_feed_name $feed(database)]
	[namespace current]::debug_log "XML parsed successfully, data length: [string length $data]" $feed_name
	...
	# Read old data BEFORE writing new data to database (for automatic updates only)
	if {![info exists feed(manual-trigger)] || $feed(manual-trigger) != 1} {
		...
		[namespace current]::debug_log "Loaded old data for comparison, length: [string length $odata]" $feed_name
	} else {
		[namespace current]::debug_log "Manual trigger - skipping old data comparison (odata will be empty)" $feed_name
	}

	[namespace current]::debug_log "Checking feed_info" $feed_name
	if {![[namespace current]::feed_info $data]} {
	...
	[namespace current]::debug_log "feed_info check passed" $feed_name

	::http::cleanup $token

	[namespace current]::debug_log "Attempting to write database to: $feed(database)" $feed_name
	...
	[namespace current]::debug_log "Database written successfully" $feed_name

	# Notify user if this was a manual trigger
	[namespace current]::debug_log "Checking for manual trigger, manual-trigger exists: [info exists feed(manual-trigger)]" $feed_name
	if {[info exists feed(manual-trigger)] && $feed(manual-trigger) == 1} {
		[namespace current]::debug_log "Manual trigger detected, finding feed name" $feed_name
		[namespace current]::debug_log "Looking for database: $feed(database)" $feed_name
		set feed_name ""
		set rss_array_names [array names ::rss-synd::rss]
		[namespace current]::debug_log "Available feeds: $rss_array_names" $feed_name
		foreach name $rss_array_names {
			array set tmp_feed $::rss-synd::rss($name)
			[namespace current]::debug_log "Checking feed '$name', database: $tmp_feed(database)" $feed_name
			if {$tmp_feed(database) == $feed(database)} {
				set feed_name $name
				[namespace current]::debug_log "Match found! Feed name: $feed_name" $feed_name
				break
			}
			unset tmp_feed
		}
		[namespace current]::debug_log "Found feed name: $feed_name" $feed_name
		if {$feed_name != ""} {
			set notify_chan ""
			...
			[namespace current]::debug_log "Notify channel: $notify_chan" $feed_name
			if {$notify_chan != ""} {
				# For manual triggers, use trigger-output instead of announce-output
				set original_announce $feed(announce-output)
				[namespace current]::debug_log "trigger-output exists: [info exists feed(trigger-output)], value: [expr {[info exists feed(trigger-output)] ? $feed(trigger-output) : "N/A"}]" $feed_name
				if {[info exists feed(trigger-output)] && $feed(trigger-output) > 0} {
					...
					[namespace current]::debug_log "Calling feed_output with announce-output=$feed(announce-output), channels=$feed(channels), odata length: [string length $odata]" $feed_name
					# Output the feed items immediately
					# For manual triggers, ensure odata is empty to show all items
					if {[info exists feed(manual-trigger)] && $feed(manual-trigger) == 1} {
						set odata ""
						[namespace current]::debug_log "Manual trigger confirmed - forcing odata to empty" $feed_name
					}
					if {[catch {[namespace current]::feed_output $data $odata} err]} {
						...
						[namespace current]::debug_log "feed_output completed successfully" $feed_name
					}
					set feed(announce-output) $original_announce
				} else {
					[namespace current]::debug_log "trigger-output is 0 or doesn't exist, sending success message" $feed_name
		...
		set feed_name [[namespace current]::find_feed_name $feed(database)]
		[namespace current]::debug_log "Regular automatic feed update, announce-output=$feed(announce-output)" $feed_name
		...
		set feed_name [[namespace current]::find_feed_name $feed(database)]
		[namespace current]::debug_log "No output (announce-output=$feed(announce-output), manual-trigger=[expr {[info exists feed(manual-trigger)] ? $feed(manual-trigger) : "N/A"}])" $feed_name
	}
}
...
proc ::rss-synd::feed_output {data {odata ""}} {
	...
	set feed_name [[namespace current]::find_feed_name $feed(database)]
	[namespace current]::debug_log "feed_output: Found $count items in feed, announce-output limit: $feed(announce-output), odata empty: [expr {$odata == ""}]" $feed_name
	...
	for {set i 0} {($i < $count) && ($i < $feed(announce-output))} {incr i} {
		...
		# Only compare against old data if odata is not empty (automatic updates)
		# For manual triggers with empty odata, skip comparison and show all items
		if {($odata != "") && [[namespace current]::feed_compare $odata $tmpd]} {
			[namespace current]::debug_log "Item $i matched old data, stopping comparison" $feed_name
			break
		}

		set tmp_msg [[namespace current]::cookie_parse $data $i]
		if {[string trim $tmp_msg] == ""} {
			[namespace current]::debug_log "Item $i produced empty message, skipping" $feed_name
			continue
		}
	...
	if {[llength $msgs] == 0} {
		# This is only a warning for automatic updates, not an error
		# For manual triggers with empty odata, this shouldn't happen unless items are empty
		if {$odata == ""} {
			[namespace current]::error_log "feed_output: No messages generated - feed items may be empty or malformed"
		} else {
			putlog "\002RSS Info\002: No new items to display (all items already in database)"
		}
		return
	}
